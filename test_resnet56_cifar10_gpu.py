...
import dill
with open("_models/train_resnet56_cifar10_gpu_(network).bn", "r") as f:
...
branchyNet.to_cpu()
with open("_models/test_resnet56_cifar10_gpu_(b_baseacc).pkl", "w") as f:
    dill.dump({'b_baseacc': b_baseacc}, f)
with open("_models/test_resnet56_cifar10_gpu_(b_basediff).pkl", "w") as f:
    dill.dump({'b_basediff': b_basediff}, f)
with open("_models/test_resnet56_cifar10_gpu_(b_num_exits).pkl", "w") as f:
    dill.dump({'b_num_exits': b_num_exits}, f)
with open("_models/test_resnet56_cifar10_gpu_(b_accbreakdowns).pkl", "w") as f:
...
branchyNet.to_cpu()
with open("_models/test_resnet56_cifar10_results_GPU_(g_ts).pkl", "w") as f:
    dill.dump({'g_ts': g_ts}, f)
with open("_models/test_resnet56_cifar10_results_GPU_(g_accs).pkl", "w") as f:
    dill.dump({'g_accs': g_accs}, f)
with open("_models/test_resnet56_cifar10_results_GPU_(g_diffs).pkl", "w") as f:
    dill.dump({'g_diffs': g_diffs}, f)
with open("_models/test_resnet56_cifar10_results_GPU_(g_exits).pkl", "w") as f:
