...
def get_network(percentTrainKeeps=1, n_class=10):
    network = [
        ...
        L.BatchNormalization(512),
        FL(F.ReLU()),
        Branch([L.Linear(None, n_class)]),  # 11
        L.Convolution2D(512, 512, 3, stride=1, pad=1),
        L.BatchNormalization(512),
        FL(F.ReLU()),
        Branch([L.Linear(None, n_class)]),  # 12
        L.Convolution2D(512, 512, 3, stride=1, pad=1),
        L.BatchNormalization(512),
        FL(F.ReLU()),
        FL(F.max_pooling_2d, 2, 2),
        SL(FL(F.dropout, 0.25, train=True)),
        Branch([L.Linear(None, n_class)]),  # 13

        L.Linear(None, 4096),
        FL(F.ReLU()),
        SL(FL(F.dropout, 0.5, train=True)),
        Branch([L.Linear(None, n_class)]),  # 14
        ...
        #L.Linear(4096, 1000),
        Branch([L.Linear(None, n_class)])  # 15
    ]
