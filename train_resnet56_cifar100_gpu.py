...
branchyNet = resnet_cifar.get_network(n_class=100, n_layer=9)
...
import dill
branchyNet.to_cpu()
with open("_models/train_resnet56_cifar100_gpu_(network).bn", "w") as f:
    dill.dump(branchyNet, f)
with open("_models/train_resnet56_cifar100_gpu_(main_loss).pkl", "w") as f:
    dill.dump({'main_loss': main_loss}, f)
with open("_models/train_resnet56_cifar100_gpu_(main_acc).pkl", "w") as f:
    dill.dump({'main_acc': main_acc}, f)
with open("_models/train_resnet56_cifar100_gpu_(main_time).pkl", "w") as f:
    dill.dump({'main_time': main_time}, f)
with open("_models/train_resnet56_cifar100_gpu_(branch_loss).pkl", "w") as f:
    dill.dump({'branch_loss': branch_loss}, f)
with open("_models/train_resnet56_cifar100_gpu_(branch_acc).pkl", "w") as f:
    dill.dump({'branch_acc': branch_acc}, f)
with open("_models/train_resnet56_cifar100_gpu_(branch_time).pkl", "w") as f:
